...
#that leads to the generation of the species whole optic lobe data in the aedes_OL.rds 
#It also contains the choice of the medulla neuroblasts cluster based on markers aedes_medulla_OL.rds.
...
larval1.data <- Read10X(data.dir = "/Aaegypti_larva1_GeneExt_merged_subsample100m/outs/filtered_feature_bc_matrix/")

larval2.data <- Read10X(data.dir = "/Aaegypti_larva2_GeneExt_merged_subsample100m/outs/filtered_feature_bc_matrix/")
...
Vln_thres <- function(obj) {
  VlnPlot(obj, "nFeature_RNA", pt.size = 0) +
    geom_hline(yintercept = 400, # or different
               linetype = "dashed",
               color = "red", size = 1) +
    guides(fill = "none")
}
...
#### subset and save ####
larv1 <- subset(larv1, subset = nFeature_RNA > 400)
...
larv2 <- subset(larv2, subset = nFeature_RNA > 400)
...
larv3 <- subset(larv3, subset = nFeature_RNA > 400)
...
saveRDS(larvaOL.integrated, "/aedes_OL.rds")
larvaOL.integrated <- readRDS("/aedes_OL.rds")
...
FeaturePlot(larvaOL.integrated, features = "rna_LOC5570443") #esg/snail/wor NBs, from Table S1 AAEL002451-PB=no expr,AAEL004154-PA=no expr,AAEL008336-PB=LOC5570443 (from uniprot-->vectorbase-->ncbi)
FeaturePlot(larvaOL.integrated, features = "rna_dpn",order=T) # NBs
FeaturePlot(larvaOL.integrated, features = "rna_LOC5575090", raster = F) #hth, epithelium or begin of medulla neuroblasts
FeaturePlot(larvaOL.integrated, features = "rna_LOC5575091", raster = F) #hth
FeaturePlot(larvaOL.integrated, features = c("rna_mira","rna_LOC5570443","rna_dpn")) #all NB markers

# find lamina and lobula plate gcm, eya, sim, tll, dac, acj6
FeaturePlot(larvaOL.integrated, features = c("rna_LOC5564850","rna_eya","rna_sim","rna_tll","rna_dac","rna_acj6")) # from TABLE S1 AAEL000631-PB, uniprot-> 5564850=gcm
...
saveRDS(NbS, "/aedes_medulla_NBs.rds")

# remove low quality clusters (that do not integrate properly among the three libraries) from aedes_OL.rds for visualization
#no removal needed for aedes
